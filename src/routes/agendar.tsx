...
function BookingPage() {
  ...
  return <div className="min-h-screen"><DropHeader/><main className="flow-page">
    <div className="flow-meta"><Link to="/" className="flex items-center gap-2 text-xs font-bold uppercase text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4"/> Voltar ao site</Link>{step<5&&<span className="text-xs font-bold uppercase text-muted-foreground">Etapa {step+1} de 5</span>}</div>
    <div className="flow-progress grid-cols-5">{[0,1,2,3,4].map(i=><div key={i} className={i<=step?'flow-progress-active':''}/>)}</div>
    <section className="flow-shell">
      {step===0&&<div className="grid min-h-[610px] md:grid-cols-[.86fr_1.14fr]"><div className="relative min-h-80 overflow-hidden"><img src={heroImage} alt="Experiência DROP Barbearia" width="1024" height="1280" className="absolute inset-0 h-full w-full object-cover"/><div className="image-shade absolute inset-0"/><span className="absolute bottom-6 left-6 text-xs font-bold uppercase">DROP · Cachoeiro</span></div><div className="flex flex-col justify-center border-t border-line p-7 sm:p-10 md:border-l md:border-t-0 lg:p-14"><p className="eyebrow-line"><span/> Bem-vindo à DROP</p><h1 className="mt-6 font-display text-4xl uppercase leading-[.9] sm:text-6xl">Seu momento.<br/><span className="text-primary">Seu estilo.</span></h1><p className="mt-6 max-w-md leading-relaxed text-muted-foreground">Escolha quem cuida do seu visual, os serviços e o melhor horário. Direto, sem complicação.</p><Button onClick={next} size="lg" className="mt-8 w-fit">Começar <ArrowRight/></Button></div></div>}
...
function Step({eyebrow,title,children}:{eyebrow:string;title:string;children:React.ReactNode}){return <div className="min-h-[530px] p-6 sm:p-9 lg:p-12"><p className="eyebrow-line"><span/>{eyebrow}</p><h1 className="mt-5 font-display text-3xl uppercase leading-none sm:text-5xl">{title}</h1><div className="mt-8">{children}</div></div>}